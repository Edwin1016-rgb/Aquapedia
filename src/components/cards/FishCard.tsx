    ...
    >
      ...
      <div className="p-3">
        <div className="flex items-center justify-between">
          <h3 className={`text-base font-semibold text-gray-900 dark:text-white ${rarity.epic ? 'rarity-epic' : ''}`}>{fish.commonName}</h3>
          <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${rarity.bg} ${rarity.text} shrink-0`}>
            {rarity.label}
          </span>
        </div>

        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 italic truncate">{fish.scientificName}</p>

        <div className="mt-2 flex gap-1 flex-wrap">
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300">{fish.diet}</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300">{fish.temperament}</span>
        </div>
      </div>
    </article>
  );
